...
def train_and_evaluate(train_dir, eval_dir, config, model_dir=None):
    ...
    Args:
        train_dir (string): Path of the train directory.
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: input_fn(train_dir, transform=True, repeat=True, shuffle=False),
        max_steps=int(config['RUN_CONFIG']['train_steps'])
    )

    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: input_fn(train_dir, transform=True, repeat=False, shuffle=False),
